...
public class LList<T> implements ListInterface<T>
{
   ...
   {      	
	  ...
	  T originalEntry = getNodeAt(givenPosition).getData();
	  Node newNode = new Node(newEntry);
	  if(givenPosition==1) {
		  Node nodeBefore = getNodeAt(givenPosition-1); 
	      newNode.setNextNode(getNodeAt(givenPosition+2));	 
	      nodeBefore.setNextNode(newNode);
	  }
	  else {
	  Node nodeBefore = getNodeAt(givenPosition - 1);    
      newNode.setNextNode(getNodeAt(givenPosition+1));
      nodeBefore.setNextNode(newNode);
	  }
